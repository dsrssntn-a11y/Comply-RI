...
**Pending action items (re-opened Aug 2026):**
1. **Emailed RIDEM again — Nathan Arruda (nathan.arruda@dem.ri.gov)** and **Tyler Hertzwig (tyler.hertzwig@dem.ri.gov)** — for re-confirmation of facility data and agricultural permit status. Awaiting reply as of Aug 2026. Their prior confirmations (Nathan, facility data, June 2026; Tyler, Greene Farm LLC's permit, July 2026 — both detailed below) remain the last-verified data until this new round comes back.
2. Update `src/data/facilities.ts` / `rhodewaste_facilities.json`, the footer dates, and this document's changelog once this reply arrives — don't let it sit unactioned.

**Facility data reverified with RIDEM (Jun 2026):** Nathan Arruda confirmed the facility inventory. Footer (`Footer.tsx`) updated from "Facility data last verified: May 2026" to "June 2026" to reflect this. **Re-confirmation requested again Aug 2026 (see Pending action items above) — not yet replied.**

**Greene Farm LLC — confirmed with Tyler Hertzwig (Jul 2026):** permit is active, issued 11/22/2024. RI agricultural composting permits have no renewal period — a new permit is only required if the farm increases compostable tonnage brought onto the farm, changes location, or changes ownership. Greene Farm brings in food waste as part of their compost recipe (also brings in plant material), confirming putrescibles are genuinely accepted there, not just nominally listed. `rhodewaste_facilities.json`/`.csv` updated accordingly — no front-facing UI change needed, since facility `notes` are maintainer-internal only (see data-hygiene note above). **Re-confirmation requested again Aug 2026 (see Pending action items above) — not yet replied.**
